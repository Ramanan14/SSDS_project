...
def run_experiment(split='iid'):
    ...
    methods = ['ewc','lwf','si']
    results = {m: {'accuracy': [], 'forgetting': []} for m in methods}
    for m in methods:
            ...
            for rnd in range(federated_rounds):
                ...
                for cid, idxs in enumerate(client_idxs):
                    ...
                    for _ in range(local_epochs):
                        for x,y in loader:
                            opt.zero_grad()
                            loss = F.cross_entropy(local(x.to(device)), y.to(device))
                            if m == 'ewc':
                    ...
                    client_ws.append(local.state_dict())
                if client_ws:
                    global_model.load_state_dict(average_weights(client_ws))
                final_acc = evaluate(global_model, test_loader)
            results[m]['accuracy'].append(final_acc)
            if inc == 0:
                results[m]['forgetting'].append(0)
            else:
                drops = [prev_acc[j] - evaluate(global_model, get_loader(tasks[j], train=False))
                         for j in range(inc)]
                results[m]['forgetting'].append(sum(drops)/len(drops))
            prev_acc.append(final_acc)
    return results
