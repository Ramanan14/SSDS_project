...
def run_experiment_fedprox(split='iid'):
    ...
    for method in methods:
        print(f"\n=== Method: {method.upper()} | FedProx | Split={split} ===")
        ...
        for inc in range(num_increments):
            ...
            new = tasks[inc]
            print(f"\n--- Increment {inc+1}/{num_increments}: new={new} ---")
            ...
            for cid, idxs in enumerate(client_idxs):
                ...
                for _ in range(local_epochs):
                    for x,y in loader:
                        ...
                        prox = 0
                        for (n, lp), (_, gp) in zip(local_model.named_parameters(),
                ...
                print(f" Client {cid} acc on seen: {acc:.4f}")
            ...
            print(f" Global acc on seen: {gacc:.4f}")
            results[method].append(gacc)
