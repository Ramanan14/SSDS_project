...
def run_experiment(split='iid'):
    ...
    for method in methods:
        ...
        global_model=None
        method_res={'global_accs':[], 'forget_rates':[], 'rounds_to_80':[]}
        for inc in range(num_increments):
            seen=sum(tasks[:inc+1],[])
            new=tasks[inc]
            # prepare old accuracy
            if global_model:
                old_loader = get_loader([i for i in range(len(full_test.targets)) if full_test.targets[i] in sum(tasks[:inc],[])])
            ...
            train_loader_all=get_loader([i for i in range(len(full_train.targets)) if full_train.targets[i] in seen],True)
            if method=='ewc':
            ...
            for r in range(federated_rounds):
                ...
                for cid,idxs in enumerate(client_idxs):
                    my=[i for i in idxs if full_train.targets[i] in seen]
                    if not my: continue
                ...
                # eval
                test_loader=get_loader([i for i in range(len(full_test.targets)) if full_test.targets[i] in seen],False)
                gacc=evaluate(global_model,test_loader)
                round_accs.append(gacc)
                # check 80% UB
                if reached_round is None and gacc>=0.8*centralized_ub[inc]:
                    reached_round=r+1
            # post-forgetting
            ...
            method_res['rounds_to_80'].append(reached_round or federated_rounds)
            print(f"Inc {inc+1}/{num_increments}: rounds accs={np.round(round_accs,3)}, forget={forget:.2f}%, rounds80={method_res['rounds_to_80'][-1]}")
