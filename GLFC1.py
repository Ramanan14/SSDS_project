...
for inc in range(num_increments):
    ...
    for _ in range(50):
        for x,y in loader_tr:
            ...
            opt.step()
    ub = sum((central(x.to(device)).argmax(1)==y.to(device)).sum().item()
             for x,y in loader_te)/len(loader_te.dataset)
    ub_accs.append(ub)
...
def run_glfc_with_metrics(split='iid'):
    ...
    # Metric containers
    round_accs   = []  # per-inc: list of 15 accuracies
    to_80        = []  # federated round to hit 0.8*UB (or 15 if never)
    forgetting   = []  # avg forgetting at end of each inc
    prev_final   = []
    ...
    for inc in range(num_increments):
        ...
        # init/expand global model
        if global_model is None:
            global_model = ResNet32(len(seen))
        else:
            global_model.fc = nn.Linear(global_model.fc.in_features,
                                        len(seen)).to(device)
        ...
        for rnd in range(1, federated_rounds+1):
            ...
            for cid, idxs in enumerate(client_idxs):
                ...
                if not my: continue
                loader = DataLoader(Subset(full_train,my),
                                     batch_size, shuffle=True)
                local = copy.deepcopy(global_model)
                opt   = optim.SGD(local.parameters(),
                                  lr=lr, momentum=0.9)
                st    = states[cid]
                initW = {n:p.clone().to(device)
                ...
                for _ in range(local_epochs):
                    for x,y in loader:
                        ...
                        for n,p in local.named_parameters():
                            loss += beta_ewc * (st['fisher'][n] *
                                               (p-st['old_params'][n]).pow(2)).sum()
                        # distill
                        ...
                        if st['exemplars']:
                            ...
                            loss += gamma_replay * F.cross_entropy(local(xe),ye)
                        # prox (FedProx term)
                        prox = sum((lp-gp).pow(2).sum()
                                   for (_,lp),(_,gp)
                                   in zip(local.named_parameters(),
            ...
            this_rounds.append(gacc)
            print(f" Round {rnd:02d}: Global Acc={gacc:.4f}")

            if hit_rnd is None and gacc>=threshold:
                hit_rnd = rnd

        round_accs.append(this_rounds)
        to_80.append(hit_rnd or federated_rounds)
        ...
        if inc==0:
            ...
            drops = []
            for j in range(inc):
                loader_j = get_loader_for_classes(tasks[j], False)
                acc_now  = evaluate(global_model, loader_j)
                drops.append(prev_final[j] - acc_now)
            forgetting.append(sum(drops)/len(drops))
        prev_final = this_rounds[-1] if inc==0 else prev_final
...
if __name__=="__main__":
    print("=== IID GLFC Metrics ===")
    m_iid = run_glfc_with_metrics('iid')
    print("\n=== Non-IID GLFC Metrics ===")
    m_noniid = run_glfc_with_metrics('noniid')
