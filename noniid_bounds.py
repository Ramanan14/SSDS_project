...
central_epochs  = 50          # centralized upper-bound
local_epochs    = 5           # local lower-bound on new classes
batch_size      = 128
lr              = 0.01
dirichlet_alpha = 0.5         # controls non‐IID skew
...
# ----------------------------
# 3) Build non-IID client splits (Dirichlet)
# ----------------------------
def create_dirichlet_split(labels, num_clients, alpha):
...
client_indices = create_dirichlet_split(
    full_train.targets, num_clients=num_clients, alpha=dirichlet_alpha
)
...
# ----------------------------
# 7) Class‐incremental loop
# ----------------------------
for t in range(num_increments):
    seen = sum(tasks[:t+1], [])  # classes that have arrived so far
    new  = tasks[t]              # newly arrived classes
    print(f"\n=== Increment {t+1}/{num_increments} | seen={seen} | new={new} ===")

    # --- Centralized upper-bound on ALL seen classes ---
    ctr_train_idx = [i for i,l in enumerate(full_train.targets) if l in seen]
    ctr_train_ds  = Subset(full_train, ctr_train_idx)
    ctr_loader    = DataLoader(ctr_train_ds, batch_size, shuffle=True, num_workers=2)

    ctr_test_idx  = [i for i,l in enumerate(full_test.targets)  if l in seen]
    ...
    print(f"[Centralized]   Acc on seen = {acc_c:.4f}")

    # --- Local lower-bound on NEW classes only ---
    local_accs = []
    for cid, idxs in enumerate(client_indices, start=1):
        my_idxs = [i for i in idxs if full_train.targets[i] in new]
        if not my_idxs:
            print(f" Client {cid:02d}: no new data → skip")
            continue
    ...
    if local_accs:
        print(f"[Local-only avg] Acc = {np.mean(local_accs):.4f}")
    else:
        print("[Local-only] No client saw the new classes")
