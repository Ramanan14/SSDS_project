...
def evaluate_per_class(model, loader, classes):
    model.eval()
...
def run_scaffold(method, split='iid'):
    ...
    for inc in range(num_increments):
        ...
        Ci = [{n:torch.zeros_like(p,device=device) for n,p in global_model.named_parameters()} for _ in range(num_clients)]
        for rnd in range(num_rounds):
            ...
            for cid, idxs in enumerate(client_idxs):
                ...
                if not idxs_new:
                    continue
                loader = DataLoader(Subset(full_train, idxs_new), batch_size, shuffle=True)
        ...
        per_class = evaluate_per_class(global_model, test_loader, seen)
        for cls in new:
            initial_acc[cls] = per_class[cls]
        if inc > 0:
            old = sum(tasks[:inc], [])
            forgetting = [initial_acc[c] - per_class[c] for c in old]
            avg_forgetting = sum(forgetting) / len(forgetting)
            print(f"Increment {inc+1} avg forgetting: {avg_forgetting:.4f}")
        acc = sum(per_class[c] for c in per_class) / len(per_class)
        print(f"Increment {inc+1} global accuracy: {acc:.4f}")

if __name__ == "__main__":
    for m in ['ewc','lwf','si']:
        print(f"Running {m.upper()} with IID split")
        run_scaffold(m,'iid')
        print(f"Running {m.upper()} with non-IID split")
        run_scaffold(m,'noniid')
