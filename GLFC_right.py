...
def run_glfc(split='iid'):
    ...
    for inc in range(num_increments):
        ...
        if last_gacc is not None:
            old_classes = sum(tasks[:inc], [])
            old_loader  = get_loader(old_classes, False)
            acc_old     = evaluate(global_model, old_loader)
            forgetting = (last_gacc - acc_old) / last_gacc
            forgetting_rates.append(forgetting)

        last_gacc = gacc

    avg_forgetting = sum(forgetting_rates) / len(forgetting_rates) if forgetting_rates else 0
    print("Global accuracy per increment:", global_results)
    print("Average forgetting rate across old classes:", avg_forgetting)
