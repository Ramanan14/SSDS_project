...
def run_prox_with_metrics(split='iid'):
    ...
    methods = ['ewc','lwf','si']
    # store per-method metrics
    metrics = {m:{
        'global_accs': [],            # list of lists (per-round accuracies)
        'forgetting': [],             # avg forgetting per increment
        'rounds_to_80': []            # first round hitting ≥80% UB
    } for m in methods}
    ...
    for inc in range(num_increments):
        ...
        for _ in range(50):  # 50 epochs centralized
    ...
    for method in methods:
        ...
        # initialize per-client CL states
        ...
        for inc in range(num_increments):
            ...
            new  = tasks[inc]
            loader_te = get_loader_for_classes(seen,train=False)
            thresh80 = 0.8 * ub_accs[inc]

            # expand or init global model
            ...
            for rnd in range(1, federated_rounds+1):
                ...
                # check 80% UB
                if reached is None and gacc >= thresh80:
                    reached = rnd

            # store metrics
            metrics[method]['global_accs'].append(round_accs)
            metrics[method]['rounds_to_80'].append(reached or federated_rounds)

            # compute avg forgetting across previous increments
            if inc == 0:
                metrics[method]['forgetting'].append(0.0)
            else:
                forgets = []
                # for each previous task j<inc
                for j in range(inc):
    ...
    for m in iid_metrics:
        ...
        for inc in range(num_increments):
            print(
                ...
                f"Rounds@80%UB={iid_metrics[m]['rounds_to_80'][inc]}"
            )
    ...
    for m in noniid_metrics:
        ...
        for inc in range(num_increments):
            print(
                ...
                f"Rounds@80%UB={noniid_metrics[m]['rounds_to_80'][inc]}"
            )
